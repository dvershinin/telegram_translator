...
Daily cron (`crontab -l`) runs `scripts/daily_podcasts.sh` hourly from 04:00 through 23:00 local. A process lock prevents overlap and a per-date success marker makes every later invocation a no-op after one full successful run; failures therefore retry after a reboot or locked login Keychain without duplicating a successful day. Each retry also skips any individual podcast whose same-date digest is already `published`, so a late failure cannot regenerate successful shows. It collects shared sources once, pre-warms Voicebox, then runs `summarize` → `podcast` → `publish` independently for each show. A failed show is logged but does not starve the remaining shows, the overall run exits nonzero, and a deduplicated `system-mcp` human-action alert points to `/tmp/daily_podcasts.log`. Current order: `crosswire` → `the_stack` → `scalable_stories` → `vaske_daily`. `scalable_stories` retrieves its WordPress application password from the explicit macOS login Keychain file only for its run and unsets both credential variables afterward.
