...
Daily cron (`crontab -l`) runs `scripts/daily_podcasts.sh` once at 04:00 local using the versioned `scripts/daily_podcasts.cron` entry. The runner logs to `~/Library/Logs/telegram_translator/daily_podcasts_YYYY-MM-DD.log` (reboot-surviving, 60-day retention pruned per run); the cron redirect appends to `daily_podcasts.log` in the same directory as a fallback for output emitted before the script's own redirect takes over. Append-only redirects preserve an active run's log if another invocation loses the process lock. One logical episode date is pinned at runner startup and passed to every CLI stage and the success marker, so a long run cannot switch dates after midnight. A manual retry skips any same-date podcast whose authoritative `published_at` and local M4A artifact show that publication completed, independent of the destination-specific status string. It collects shared sources once, pre-warms Voicebox, then runs `summarize` → `podcast` → `publish` independently for each show. A failed show is logged but does not starve the remaining shows, the overall run exits nonzero, and a deduplicated `system-mcp` human-action alert points to the dated durable log. Current order: `crosswire` → `the_stack` → `scalable_stories`. `vaske_daily` is disabled as of 2026-09-14 (its ~30 extra segments pushed the 04:00 run past noon); the call is commented out in the runner and guarded by `tests/test_daily_podcasts_script.py::test_vaske_daily_stays_disabled`. `scalable_stories` reads only its WordPress application password from `~/Library/Application Support/telegram_translator/credentials/scalable-stories-wordpress-password` (owned by the runner user, directory mode `0700`, regular file mode `0600`, no symlinks or hard links). No login Keychain access is needed at runtime. The runner exports `GPS_WP_USER=danila` and `GPS_WP_APP_PASSWORD` only for that show, clears both after success or failure, and treats missing/unsafe credentials as a retryable show failure. See `docs/publishing.md` for provisioning and rotation.
...
- **`wordpress`** — Our publisher encodes M4A locally, creates/reuses a draft through WordPress REST, uploads/reuses its attached media, and then finalizes a Seriously Simple Podcasting episode. WordPress remains the RSS source of truth, which preserves historical episode GUIDs and existing directory subscribers. Credentials are application-password env vars named by `username_env` and `application_password_env`; each podcast supplies its SSP `publish.series_id`.
