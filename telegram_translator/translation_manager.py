...
class IdiomaTranslator(BaseTranslator):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        try:
            ...
            raise ImportError(
                "Idioma package is not installed. Run: pip install idioma"
            )
    ...
    async def translate(
        ...
        try:
            ...
            result = await asyncio.to_thread(
                self.translator.translate, text, src=source, dest=target
            )
...
class TranslationManager:
    ...
    def get_provider_info(self) -> Dict[str, Any]:
        ...
        if self.provider == "openai":
            ...
            return {
                "provider": "idioma",
                "source_language": self.config.get("idioma", {}).get(
                    "source_language", "auto"
                ),
                "target_language": self.config.get("idioma", {}).get(
                    "target_language", "en"
                ),
            }
        else:
